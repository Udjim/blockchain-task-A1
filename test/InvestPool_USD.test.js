const {
  ...
  describe("InvestPool real USD", function () {
    // =================================
    ...
    async function deploy() {
      ...
      const FactoryRealUSD = await ethers.getContractFactory("realUsd");
      const realUSD = await FactoryRealUSD.deploy();
      await realUSD.deployed();
  
      //Minting payment tokens
      const mintToOwner = await realUSD.mint(
        owner.address, 
        10**12
      );
      await mintToOwner.wait();
      const mintToUser = await realUSD.mint(
        user.address, 
        10**12
      );
    ...
    describe("Check initial values", function () {
      it("should sets values correctly", async function () {
        ...
        expect(realUSD.address).to.be.properAddress;
        ...
        expect(await realUSD.decimals()).to.equal(6);
        ...
        expect(await investPool.price()).to.equal(price * 10**12);
        ...
        expect(await realUSD.balanceOf(user.address)).to.equal(
            10**12
        );
    ...
    describe("buyLP() REAL USD", function () {
      it("should allows to buy and change balances", async function () {
        ...
        const userBalance = await realUSD.balanceOf(user.address);
        const txApr = await realUSD
          .connect(user)
        ...
        const realUSDDecimals = await realUSD.decimals()
        const lpTokenDecimals = await lpToken.decimals()
        const realUSDAmountWithoutFee =
          ((lpTokenAmountToBuy * price * 10 ** (realUSDDecimals - 2)) / 10 ** lpTokenDecimals) * 10**12;
        const realUSDAmount = ((realUSDAmountWithoutFee * 
      ...
      it('should sets values after buying correctly', async function () {
        ...
        expect(await investPool.alreadyBought(user.address)).to.equal(realUSDAmountWithoutFee - (realUSDAmountWithoutFee % 1));
        ...
        const totalrealUSDSpended = await investPool.totalPaymentTokenSpended();
        expect(Number(totalrealUSDSpended)).to.equal(realUSDAmount - realUSDAmount % 1);
        expect(Number(userRoleSettings2.totalAmountOfPaymentTokenSpended)).to.equal(realUSDAmount - realUSDAmount % 1);
      });
      ...
      it("should revert with LT", async function () {
        ...
        const ownerBalance = await realUSD.balanceOf(owner.address);
        const txApr = await realUSD
          .connect(owner)
